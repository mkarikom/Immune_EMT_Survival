...
mycantypes = [1,3];
nk = 2;
n_sub = nk*1000; % the number of bootstrap samples to take for each tumor
nstd = 0.01; % the nstd * standard deviation of gene n * rand(1) + gene n = the level of gene n when doing resampling
...
mkdir(plotdir);
rootdir = [plotdir,'lineplots_TryResample_' num2str(nGrid) '_'];
...
for i = 1:size(metadata,1)
    if ismember(i,mycantypes)
        ...
        tumors(i).patients = zeros(n_sub,1); % get the patient ids for each bootstrap sample

        tumors(i).patients = randsample(s,length(y),n_sub,true);

        expr = zeros(n_sub,size(data_expr_vals,2)); % 3d array Genes x Patients x Boot Samples
        base_expr = table2array(data_expr_vals(tumors(i).patients,:)); % enter a genes x patients 2d array
        noise_mat = rand(size(table2array(data_expr_vals(tumors(i).patients,:))));
        noise_multiplier = repmat(nstd * std(exp(table2array(data_expr_vals(tumors(i).patients,:))),1),n_sub,1);
        noise = noise_mat .* noise_multiplier;
        expr = table2array(data_expr_vals(tumors(i).patients,:)) + log(noise); % the non-standardized re-sampled log-expression with noise added
        dfi = y(tumors(i).patients);% the corresponding patient dfi class
        ...
        for primary = 1:M
            ...
            for secondary = 1:M
                ...
                if ~isfile([outputdir pgene '/' sgene '.pdf'])
                    %% grid data
                    allgenesMean = repmat(feval('mean',XN), nGrid, 1); % set all genes to mean standardized value across patients
        ...
        %                gpcf_c = gpcf_constant('constSigma2_prior',prior_sinvchi2('s2',1^2,'nu',1));
                        gpcf_all = gpcf_sexp(gpcf_all, 'magnSigma2_prior',prior_gaussian('mu',1,'s2',0.25),'lengthScale_prior',prior_gaussian('s2',.1)); %
