...
mkdir(plotdir);
rootdir = [plotdir,'lineplots_FullData_WithParfor_WithBox_LowReps_' num2str(nGrid) '_'];
        ...
        parfor primary = 1:M
            ...
            for secondary = 1:M
                ...
                if ~isfile([outputdir pgene '/' sgene '.pdf'])
                    ...
                    xlabel(['Log(' tumors(i).genes{primary} ')'],'FontSize',labFont);
                    ...
                    if XMEAN(primary)-1.645*XSTD(primary) >= min(x(:,primary))
                        leftx = XMEAN(primary)-1.645*XSTD(primary);
                    else
                        leftx = min(x(:,primary));
                    end
                    if XMEAN(secondary)-1.645*XSTD(secondary) >= min(x(:,secondary))
                        lefty = XMEAN(secondary)-1.645*XSTD(secondary);
                    else
                        lefty = min(x(:,secondary));
                    end
                    if XMEAN(primary)+1.645*XSTD(primary) <= max(x(:,primary))
                        rightx = XMEAN(primary)+1.645*XSTD(primary);
                    else
                        rightx = max(x(:,primary));
                    end
                    if XMEAN(secondary)+1.645*XSTD(secondary) <= max(x(:,secondary))
                        righty = XMEAN(secondary)+1.645*XSTD(secondary);
                    else
...
%% run additive model
rootdir = [plotdir,'lineplots_FullData_WithParfor_WithBox_ADDITIVE_LowReps_' num2str(nGrid) '_'];
mkdir('tmp')
tumors = struct();
