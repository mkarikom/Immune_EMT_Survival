...
tumors = struct();
setreplace = true; % whether to sample with replacement
...
maxfevals = 100;
mycantypes = [1,3];
nstd = 0.10; % the nstd * standard deviation of gene n * rand(1) + gene n = the level of gene n when doing resampling

if ~isdeployed
    addpath("../DataTables/")
end
dirname = ['./DataTables/resampling_IG_G_Replacement_' num2str(nk) 'k_',num2str(fixedresample),'n/'];
...
for i = 1:size(metadata,1)
    if ismember(i,mycantypes)
        ...
        if ~isfile([dirname 'meta_AddRecGene_' tumors(i).type{1} '.mat'])
            ...
            for ii = 1:n_boot
                if setreplace % if we sample with replacement, need to add noise to the data to prevent numerical issues
                    ...
                    expr(:,:,ii) = table2array(data_expr_vals(tumors(i).patients(:,ii),:)) + log(noise); % enter a genes x patients 2d array
                else
            ...
            for(ii = 1:n_boot)
                ...
                if ii == 15
                    fprintf('this is the one with infinite f')
                end
                ...
                opt=optimset('TolFun',basetolf,'TolX',basetolx,'MaxIter',maxiter,'MaxFunEvals',maxfevals,'Display','iter','DiffMaxChange',1e3);
                ...
                fprintf('\n ard ls \n')
                exp(gp.cf{2}.lengthScale)
                
                if gpes(ii) > 1e10
                    fprintf('warning: likelihood has issue')
                end
...
dirname = ['./DataTables/resampling_IG_G_ADDITIVE_' num2str(nk) 'k/'];
metadata = readtable("../DataTables/Prolif_acc_AddRecGene.txt", 'ReadRowNames', false, 'Delimiter', '\t');
rng(1);h
