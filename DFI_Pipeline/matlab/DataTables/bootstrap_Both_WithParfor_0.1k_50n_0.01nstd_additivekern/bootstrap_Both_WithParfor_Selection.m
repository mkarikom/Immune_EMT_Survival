...
n_boot = nk*1000; 
n_sub = 50; % the number of resampled patients per n_boot model fit
...
dirname = ['./DataTables/bootstrap_Both_WithParfor_' num2str(nk) 'k_',num2str(n_sub),'n','_',num2str(nstd),'nstd_',kerneltype,'kern/'];
...
for i = 1:size(metadata,1)
    if ismember(i,mycantypes)
        ...
        if ~isfile([dirname 'meta_AddRecGene_' tumors(i).type{1} '.mat'])
            ...
            parfor(ii = 1:n_boot)
                ...
                gpcf_all = gpcf_sexp(gpcf_all, 'magnSigma2_prior',prior_gaussian('mu',1,'s2',0.25),'lengthScale_prior',prior_gaussian('mu',1,'s2_prior',prior_invgamma('sh',1,'s',1))); %
 ...
 %               [gps(ii).W,gps(ii).WH,gps(ii).H] = gp_pak(gp);
            ...
            save([dirname 'bootstrap_Both_WithParfor_' tumors(i).type{1} '.mat'],'currentType');
